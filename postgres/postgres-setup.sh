...
# Set variable
export HARBOR_URL=registry.pivotal.io
export HARBOR_USER=sapphire
export HARBOR_PASSWORD=test123
...
kubectl create secret docker-registry image-pull-secret --namespace=gemfire-system --docker-server=$HARBOR_URL --docker-username=$HARBOR_USER --docker-password=$HARBOR_PASSWORD

kubectl create secret docker-registry image-pull-secret --docker-server=$HARBOR_URL --docker-username=$HARBOR_USER --docker-password=$HARBOR_PASSWORD
...
helm registry login $HARBOR_URL --username=$HARBOR_USER --password=$HARBOR_PASSWORD

helm chart pull $HARBOR_URL/tanzu-sql-postgres/postgres-operator-chart:v1.2.0
helm chart export $HARBOR_URL/tanzu-sql-postgres/postgres-operator-chart:v1.2.0  --destination=/tmp/

kubectl create secret docker-registry regsecret \
--docker-server=https://$HARBOR_URL --docker-username=$HARBOR_USER \
